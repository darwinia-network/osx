    ...
    DaoAuthorizableUpgradeable
{
    ...
    function delegate(address delegatee) public virtual override {
        // Automatically turn on delegation for delegatee, but only for the first time.
        if (
            delegatee != address(0) &&
            numCheckpoints(delegatee) == 0 &&
            delegates(delegatee) == address(0)
        ) {
    ...
    function _afterTokenTransfer(address from, address to, uint256 amount) internal override {
        ...
        // Potencial additional feature
        // Reset delegation of `from` to itself when balance && votes are zero.
        // If `from` balance and votes went to zero, we reset its previous delegation to itself,
        // so in the future when `from` aquire new balance it will have self-delegation.
        // if (balanceOf(from) == 0 && getVotes(from) == 0) {
        //     _delegate(from, from);
        // }
    }
}
