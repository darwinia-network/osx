...
describe.only('GovernanceERC20 V2', function () {
  ...
  describe('Delegation Enabler bug', async () => {
    it('should enable auto delegation when receive tokens', async () => {
      const address1 = signers[0].address;
      const address2 = signers[5].address;
      ...
      console.log('-----------------------------------');
      let balanceAddress1 = await token.balanceOf(address1);
      let balanceAddress2 = await token.balanceOf(address2);

      console.log('address-1 balance', balanceAddress1.toString());
      console.log('address-2 balance', balanceAddress2.toString());

      let votesAddress1 = await token.getVotes(address1);
      let votesAddress2 = await token.getVotes(address2);

      console.log('address-1 votes', votesAddress1.toString());
      console.log('address-2 votes', votesAddress2.toString());

      let delegates1 = await token.delegates(address1);
      let delegates2 = await token.delegates(address2);

      console.log('address-1 delegates ->', delegates1.toString());
      console.log('address-2 delegates ->', delegates2.toString());

      // delegate from address-1 -> address-2
      await token.connect(signers[0]).delegate(address2);

      console.log('-----------------------------------');
      console.log('After delegate from address-1 -> address-2 state:');
      console.log('-----------------------------------');
      balanceAddress1 = await token.balanceOf(address1);
      balanceAddress2 = await token.balanceOf(address2);

      console.log('address-1 balance', balanceAddress1.toString());
      console.log('address-2 balance', balanceAddress2.toString());

      votesAddress1 = await token.getVotes(address1);
      votesAddress2 = await token.getVotes(address2);

      console.log('address-1 votes', votesAddress1.toString());
      console.log('address-2 votes', votesAddress2.toString());

      delegates1 = await token.delegates(address1);
      delegates2 = await token.delegates(address2);

      console.log('address-1 delegates ->', delegates1.toString());
      console.log('address-2 delegates ->', delegates2.toString());

      console.log('-----------------------------------');
      console.log('Transferring token to address-2 to enable delegation');
      console.log('-----------------------------------');
      await token.connect(signers[0]).transfer(address2, 23);
      ...
      expect(await token.delegates(address2)).to.be.eq(address2);
    });
  });
